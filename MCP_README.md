...
This VS Code extension includes a comprehensive Model Context Protocol (MCP) server that provides **87 tools** for interacting with the AppDNA model.

## Features

### **87 Comprehensive Tools** ✅ Verified with GitHub Copilot
...
8. **get_model_validation_request_details** - Get detailed information for a specific validation request by request code
9. **get_model_validation_request_schema** - Get JSON schema definition for validation request objects
10. **list_fabrication_blueprint_catalog_items** - List available fabrication blueprints (template sets) with selection status
11. **select_fabrication_blueprint** - Add a fabrication blueprint from the catalog to your AppDNA model (requires name AND version)
12. **unselect_fabrication_blueprint** - Remove a fabrication blueprint from your AppDNA model (requires name AND version)
13. **list_model_fabrication_requests** - List fabrication requests with status and download information
14. **get_model_fabrication_request_details** - Get detailed information for a specific fabrication request by request code
15. **get_model_fabrication_request_schema** - Get JSON schema definition for fabrication request objects
