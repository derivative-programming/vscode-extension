...
<head>
    ...
    <title>Workflow Task Details: ${flowName}</title>
...
<body>
    <div class="header-container">
        <h1 class="header-title">Details for ${flowName} Workflow Task</h1>
        <button class="copy-workflow-task-name-button" onclick="copyWorkflowTaskName('${flowName || ''}')" title="Copy Workflow Task Name">
    ...
    <script>
        ...
        function copyWorkflowTaskName(flowName) {
            if (!flowName) { return; }
            try {
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    navigator.clipboard.writeText(flowName).then(() => {
                        const icon = document.querySelector('.copy-workflow-task-name-button .codicon');
                        if (icon) {
