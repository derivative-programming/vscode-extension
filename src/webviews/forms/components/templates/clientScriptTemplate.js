 ...
 */
function getClientScriptTemplate(params, buttons, outputVars, paramSchema, buttonSchema, outputVarSchema, formName, allForms = [], allReports = [], allDataObjects = []) {
    return `
        (function() {
            ...
            window.openPageInitDetails = function(pageInitName) {
                console.log('[DEBUG] FormDetails - Opening page init details for page init name:', JSON.stringify(pageInitName));
                
                if (!pageInitName) {
                    console.error('[ERROR] FormDetails - No page init name provided');
                    return;
                }
            ...
            function getDataObjectSearchModalHtml() {
                return \`${getDataObjectSearchModalHtml().replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`;
            }
