...
describe("AppDNA Clean Project Test", function () {
    ...
    it("should have the extension icon in the activity bar", async function () {
        ...
        assert.ok(viewContainers && viewContainers.length > 0, "Extension should contribute a view container to the activity bar");
        // Print information about the view container
        console.log("View container in activity bar:", viewContainers.map((vc) => `${vc.id} (${vc.title})`));
    ...
    it("should focus the extension view when clicking the icon", async function () {
        ...
        try {
            ...
            assert.ok(appDnaCommands.includes("appdna.addFile"), "addFile command should be available after focusing the view");
        }
        catch (err) {
    ...
    it("should verify context menu actions available", async function () {
        try {
            ...
            const commands = await vscode.commands.getCommands(true);
            const contextMenuCommands = commands.filter(cmd => cmd.startsWith("appdna.") &&
            ...
            assert.ok(commands.includes("appdna.saveFile"), "Save File command should be available after creating app-dna.json");
        }
        catch (err) {
...
//# sourceMappingURL=clean-project.test.js.map